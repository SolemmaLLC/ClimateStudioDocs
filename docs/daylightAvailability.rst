...
The display settings are very flexible. For example, the figure below shows the illuminance distribution on July 11 at 5:30am for a sensor near the South facing facade. (To select a sensor, just hover the mouse over it.) The lower figure on the right figure shows the so-called, temporal map for the sensor with illuminances, generally peaking from mid April to mid August.
...
As of summer 2020, the US Green Building Council accepted ClimateStudio report for compliance for LEEDv4 Daylighting Credit option 1. An example report can be `downloaded here.`_
